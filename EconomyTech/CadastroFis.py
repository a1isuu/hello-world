import re
import os
from ValidaCPF import validacaocpf
...
def cadastro():
    ...
    while (resposta != 'Y') or (resposta != 'y'):
        ...
        valida = validacaocpf(cpf)
        while (valida == 'CPF inválido'):
            print('Este CPF é inválido, por favor reescreva-o.')
            cpf = input('Insira seu CPF (sem as pontuações):\n')
            if (len(cpf) <= 10) or (len(cpf) > 11):
                while len(cpf) != 11:
                    print('Tamanho inválido.')
                    cpf = input('Insira-o novamente (sem as pontuações):\n')
            valida = validacaocpf(cpf)
        resposta = input(cpf[:3]+'.'+cpf[3:6]+'.'+cpf[6:9]+'-'+cpf[9:]+' esta correto?\nY - Para sim, N - Para não.\n')
        if (resposta == 'Y') or (resposta == 'y'):
            break
        if (resposta != 'Y') and (resposta != 'y') and (resposta != 'N') and (resposta != 'n'):
            while (resposta != 'Y') and (resposta != 'y') or (resposta != 'N') and (resposta != 'n'):
                resposta = input('Opção inválida, por favor insira Y ou N.\n')
                if (resposta == 'Y') or (resposta == 'y') or (resposta == 'N') or (resposta == 'n'):
                    break
        if (resposta == 'Y') or (resposta == 'y'):
    ...
    while (resposta != 'Y') or (resposta != 'y'):
        ...
        if (len(numero) < 9) or (len(numero) > 9):
            ...
            while (len(numero) < 9) or (len(numero) > 9):
                ...
                ddd = str(telefone[:3])
                numero = str(telefone[3:])
        resposta = input(f'({ddd}){numero[:5]}-{numero[5:]}'+' é o seu número de telefone?\nY - Para sim, N - Para não.\n')
        if (resposta == 'Y') or (resposta == 'y'):
            break
        if (resposta != 'Y') and (resposta != 'y') and (resposta != 'N') and (resposta != 'n'):
    ...
    infosUs = [nomeCompleto, senha, email, dia, mes, ano, sexo, cpf, telefone, perguntaEmpr, pergTermos]

    with open('userData.txt','r+') as arquivo:
        for valor in arquivo:
            print(valor)
        arquivo.write(str(infosUs) + '\n')
