...
def cadastroJur():
    ...
    cnpj = input('Insira aqui o seu CNPJ: ')
    while(len(cnpj) < 14) or (len(cnpj) > 14):
        cnpj = input('Tamanho inválido.\nInsira novamente o seu CNPJ: ')
    ...
    while (resposta != 'Y') or (resposta != 'y'):
        ...
        ddd = str(telContato[:3])
        numero = str(telContato[3:])
        ...
        if (len(numero) < 8) or (len(numero) > 8):
            ...
            while (len(numero) < 8) or (len(numero) > 8):
                print('Seu número de telefone é inválido.')
                telContato = str(input('Digite-o novamnente (DDD e número):\n'))
                ddd = str(telContato[:3])
                numero = str(telContato[3:])
        print(f'({ddd}){numero[:4]}-{numero[4:]}')
    ...
    while (porteEmpresa != '1') and (porteEmpresa != '2') and (porteEmpresa != '3'):
        porteEmpresa = str(input('Indique o porte da empresa:\n1 - MEI\n2 - ME\n3 - EPP\n'))
    ...
    infosJur = [nomeEmpresa, emailEmpresa, nomeRua, numLocal, bairro, cidade, estado, cep, cnpj, telContato, tipoEmpresa, numFuncionario, porteEmpresa]

    with open('jurData.txt','r+') as arquivo:
        for valor in arquivo:
            print(valor)
        arquivo.write(str(infosJur) + '\n')
